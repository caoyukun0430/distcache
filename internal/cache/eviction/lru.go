...
func (c *CacheUseLRU) cleanupRoutine() {
	...
	for {
		...
		case <-ticker.C:
			...
			loggerInstance.Infof("cleanupRoutine triggered after %v", c.cleanupInterval)
...
func (c *CacheUseLRU) CleanUp(ttl time.Duration) {
	...
	for _, seg := range c.segments {
		// loggerInstance.Infof("lru CleanUp triggered seg %+v at ttl %+v", seg, ttl)
...
func (seg *segment) removeElement(e *list.Element) {
	...
	// loggerInstance.Infof("lru removeEntry %+v value %+v", entry.Key, entry.Value)
	seg.nbytes -= int64(len(entry.Key)) + int64(entry.Value.Len())
	if seg.OnEvicted != nil {
		// loggerInstance.Infof("lru OnEvicted called %+v", seg.OnEvicted)
