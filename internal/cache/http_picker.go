...
func (p *HTTPPool) Log(format string, v ...interface{}) {
	loggerInstance.Infof("[Server %s] %s", p.currentServer, fmt.Sprintf(format, v...))
}
...
func (p *HTTPPool) Pick(key string) (Fetcher, bool) {
	...
	loggerInstance.Infof("[request forward by peer %s], pick remote peer: %s", p.currentServer, peerAddress)
